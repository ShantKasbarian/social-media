...
public class DefaultExceptionHandler {
  ...
  static final String INTERNAL_SERVER_ERROR_MESSAGE =
      "An unexpected error occurred. Please try again later.";

  @ExceptionHandler(InvalidCredentialsException.class)
  public ResponseEntity<ErrorDto> handle(InvalidCredentialsException e) {
    return new ResponseEntity<>(new ErrorDto(INVALID_CREDENTIALS_MESSAGE), HttpStatus.UNAUTHORIZED);
  }

  @ExceptionHandler(InvalidInputException.class)
  public ResponseEntity<ErrorDto> handle(InvalidInputException e) {
    return new ResponseEntity<>(new ErrorDto(INVALID_INPUT_MESSAGE), HttpStatus.BAD_REQUEST);
  }

  @ExceptionHandler(RequestNotAllowedException.class)
  public ResponseEntity<ErrorDto> handle(RequestNotAllowedException e) {
    return new ResponseEntity<>(new ErrorDto(REQUEST_NOT_ALLOWED_MESSAGE), HttpStatus.FORBIDDEN);
  }

  @ExceptionHandler({ResourceNotFoundException.class, NoResourceFoundException.class})
  public ResponseEntity<ErrorDto> handle(Exception e) {
    return new ResponseEntity<>(new ErrorDto(RESOURCE_NOT_FOUND_MESSAGE), HttpStatus.NOT_FOUND);
  }

  @ExceptionHandler(ResourceAlreadyExistsException.class)
  public ResponseEntity<ErrorDto> handle(ResourceAlreadyExistsException e) {
    return new ResponseEntity<>(new ErrorDto(RESOURCE_ALREADY_EXISTS_MESSAGE), HttpStatus.CONFLICT);
  }
  ...
  public ResponseEntity<ErrorDto> handle(MethodArgumentNotValidException e) {
    ...
    return new ResponseEntity<>(new ErrorDto(errorMessages.toString()), HttpStatus.BAD_REQUEST);
  }

  @ExceptionHandler(ConstraintViolationException.class)
  public ResponseEntity<ErrorDto> handle(ConstraintViolationException e) {
    var errors =
        e.getConstraintViolations().stream().map(ConstraintViolation::getMessageTemplate).toList();
