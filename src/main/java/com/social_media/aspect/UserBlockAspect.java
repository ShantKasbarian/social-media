...
public class UserBlockAspect {
  ...
  private boolean checkCreateComment(Object[] args) {
    ...
    return friendRequestRepository.existsByUserIdTargetUserId(
        user.getId(), comment.getPost().getUser().getId());
  }
  ...
  private boolean checkCreateLike(Object[] args) {
    ...
    return friendRequestRepository.existsByUserIdTargetUserIdStatus(
  ...
  private boolean checkGetUserPosts(Object[] args) {
    ...
    return friendRequestRepository.existsByUserIdTargetUserIdStatus(
        user.getId(), targetUserId, FriendRequest.Status.BLOCKED);
  }
}
